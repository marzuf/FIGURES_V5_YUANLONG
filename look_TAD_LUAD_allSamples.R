...
startTime <- Sys.time()
cat(paste0("> Rscript look_TAD_LUAD_allSamples.R\n"))

script_name <- "look_TAD_LUAD_allSamples"
...
all_fpkm_dt <- get(load("PREP_FPKM_LUAD_ALL_SAMPLES/all_luad_fpkm_dt.Rdata"))
...
outFolder <- file.path("LOOK_TAD_LUAD_ALL_SAMPLES")
...
stopifnot(rownames(fpkm_dt) %in% rownames(all_fpkm_dt))
all_fpkm_dt <- all_fpkm_dt[rownames(fpkm_dt),]

all_fpkm_dt2 <-  apply(all_fpkm_dt, 2, function(x)x/sum(x))
# stopifnot(colSums(fpkm_dt2) == 1)
stopifnot(abs(colSums(all_fpkm_dt2) - 1) <= 10^-4)
# and then multiply by 10^6 to have FPKM
all_fpkm_dt2 <- all_fpkm_dt2*10^6
all_fpkm_dt2 <- data.frame(all_fpkm_dt2, check.names = FALSE)
stopifnot(dim(all_fpkm_dt) == dim(all_fpkm_dt2))
stopifnot(rownames(all_fpkm_dt) == rownames(all_fpkm_dt2))
stopifnot(colnames(all_fpkm_dt) == colnames(all_fpkm_dt2))
all_fpkm_dt <- all_fpkm_dt2
stopifnot(names(geneList) %in% rownames(all_fpkm_dt))


dt1 <- fpkm_dt[names(plot_geneList), cond1_ID]
dt2 <- all_fpkm_dt[names(plot_geneList), cond1_ID]
...
save(fpkm_plot_dt, file="fpkm_plot_dt.Rdata", version=2)

# fpkm_plot_dt <- fpkm_plot_dt[,c("entrezID", samp1, samp2)]  # 11.03.2020: changed here -> keep all samples
