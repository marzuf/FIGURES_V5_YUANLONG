# Rscript meanLogFC_density_heatmap_v2.R
...
script_name <- "meanLogFC_density_heatmap.R"
...
outFolder <- "MEANLOGFC_DENSITY_HEATMAP_V2" 
...
if(buildData){
  ...
  all_result_dt <- foreach(hicds = all_hicds, .combine='rbind') %dopar% {
    ...
    hicds_dt <- foreach(exprds = all_exprds[[paste0(hicds)]], .combine='rbind') %do% {
      ...
      fc_file <- file.path(pipOutFolder, hicds, exprds, "3_runMeanTADLogFC", "all_meanLogFC_TAD.Rdata")
      stopifnot(file.exists(fc_file))
      all_fc <- as.numeric(get(load(fc_file)))
	  stopifnot(!is.na(all_fc))
...
for(a_t in all_types) {
  ...
  density_plot <- ggplot(plot_dt, aes(x = dataset, y = density_x, fill = density_y))+
    geom_tile() +
  ggtitle(paste0("Mean LogFC distribution - ", a_t),
    ...
    scale_y_continuous(name="Mean LogFC",
  ...
  outFile <- file.path(outFolder, paste0("meanLogFC_dist_allDS_", a_t, "_densityheatmap.", plotType))
  ggsave(density_plot, filename = outFile,  height=myHeightGG, width=myWidthGG)
  cat(paste0("... written: ", outFile, "\n"))
  
  
  sub_dt <- all_result_dt[all_result_dt$hicds_lab == a_t,]
  sub_dt$meanLogFC[sub_dt$hicds == dirname(ds_levels[1]) & sub_dt$exprds == basename(ds_levels[1])] <- sub_dt$meanLogFC[sub_dt$hicds == dirname(ds_levels[1]) & sub_dt$exprds == basename(ds_levels[1])] - 0.5
  ...
  outFile <- file.path(outFolder, paste0("meanLogFC_dist_allDS_", a_t, "_densityheatmap_checkplot.", plotType))
  ggsave(density_plot, filename = outFile,  height=myHeightGG, width=myWidthGG)
  cat(paste0("... written: ", outFile, "\n"))
