...
# Rscript nSignifGenes_nSignifTADs_minSamp_byCond.R

outFolder <- file.path("NSIGNIFGENES_NSIGNIFTADS_MINSAMP_BYCOND")
...
for(x_var in c("minNbrSample", "ds_rank")) {
...
sink()
	...
	mtext(side=3, line=-1, text=paste0(curr_cond, " - n= ", length(unique(nSignif_dt$dataset))))
	...
	outFile <- file.path(outFolder, paste0("nSignifGenes_vs_minNbrSample_all_ds_",geneSignifThresh, "_tadSignif", tadSignifThresh, "_", curr_cond, ".", plotType))
	do.call(plotType, list(outFile, height=myHeight, width=myWidth))
	par(bty="L")
	plot(
	...
	outFile <- file.path(outFolder, paste0("nSignifTADs_vs_minNbrSample_all_ds_",geneSignifThresh, "_tadSignif", tadSignifThresh, "_", curr_cond, ".", plotType))
	do.call(plotType, list(outFile, height=myHeight, width=myWidth))
	par(bty="L")
	plot(
	  x = nSignif_dt$minNbrSample,
